...
try:
  ...
  kpi2.metric(
      'Churn Rate', f'{churn_rate:.1f}%', delta='-1.2% YoY', delta_color='inverse'
  )
  ...
  tab1, tab2, tab3, tab4 = st.tabs(
      ['Overview', 'Sustainability & Logistics', 'Risk Simulator', 'Data']
  )
  ...
  with tab2:
    st.info(
        '**What is ESG (Environmental, Social, and Governance) in this'
        ' context?**\n\nIn e-commerce operations, customer dissatisfaction,'
        ' extended delivery distances, and product returns directly generate'
        ' reverse logistics mileage, increasing overall carbon (CO2) emissions.'
    )
    ...
    with col_chart1:
      ...
      opt_esg = {
          ...
          'yAxis': [{'type': 'value', 'name': 'Returns'}, {'type': 'value', 'name': 'CO2 (kg)'}],
          'series': [
              {
    ...
    with col_chart2:
      ...
      dist_comp = (
          df.groupby(['DistanceCategory', 'Complain'])
          .size()
    ...
    st.markdown(
        'Estimate potential carbon and cost reductions by resolving customer'
        ' complaints.'
    )
    ...
    with sim_col1:
      ...
      cost_per_return = st.number_input(
          'Avg Logistics Cost per Return (€):', value=12.50
      )
    ...
    with sim_col2:
      ...
      saved_co2 = (
          complaint_co2 * (complaint_reduction / 100)
      ) / 1000  # tons
      saved_cost = (
          complaint_returns * (complaint_reduction / 100) * cost_per_return
      )
      ...
      st.success(
          f'Achieving a {complaint_reduction}% reduction in complaints'
          f' eliminates {saved_co2:.1f} tons of carbon emissions from reverse'
          ' logistics.'
      )
  ...
  with tab3:
    ...
    st.markdown(
        'Simulate top model features to inspect live churn probability using'
        ' XGBoost.'
    )
    ...
    with sim_left:
      ...
      marital_val = st.selectbox(
          'Marital Status:', ['Single', 'Married', 'Divorced']
      )
      payment_val = st.selectbox(
          'Preferred Payment Mode:',
          ['Credit Card', 'E wallet', 'Debit Card', 'Cash on Delivery', 'UPI'],
      )
    ...
    with sim_right:
      input_data = pd.DataFrame({
          ...
          'PreferedOrderCat': [order_cat_val],
          'MaritalStatus': [marital_val],
          'PreferredPaymentMode': [payment_val],
          'WarehouseToHome': [15],
          'OrderCount': [2],
      ...
      try:
        ...
        risk_score = int(churn_proba * 100)
      except Exception as e:
        st.error(f'Prediction error: {e}')
        risk_score = 0

      gauge_color = '#91CC75' if risk_score < 60 else '#EE6666'
      ...
      if risk_score >= 60:
        st.error(
            'High Risk Profile: Proactive customer retention intervention'
            ' required.'
        )
      else:
  ...
  st.warning(
      'Ensure `clean_data.csv` is located at `data/processed/clean_data.csv` and'
      ' `xgb_pipeline.pkl` is saved.'
  )
