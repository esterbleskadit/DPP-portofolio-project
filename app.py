...
st.set_page_config(page_title='E-Commerce Churn & Sustainability Analytics',
                   layout='wide',
...
try:
    ...
    if 'TenureBucket' in df.columns:
        ...
        tenure_filter = st.sidebar.multiselect('Tenure:', options=tenure_opts, default=tenure_opts)
    ...
    if 'WarehouseToHome' in df.columns:
        ...
        max_dist = st.sidebar.slider('Warehouse Distance (km):', min_value=min_d,
                                                                 max_value=max_d,
                                                                 value=max_d)
    ...
    total_returns = (df['Estimated_Returns'].sum() if 'Estimated_Returns' in df.columns else 0)
    ...
    with tab1:
        ...
        with row1_col1:
            ...
            opt_donut = {'tooltip': {'trigger': 'item', 'formatter': '{b}: {c} ({d}%)'},
                         ...
                         'series': [{'name': 'Churn Status',
                                     ...
                                     'data': [{'value': churn_counts.get(0, 0), 'name': 'Retained', 'itemStyle': {'color': '#5470C6'}},
                                              {'value': churn_counts.get(1, 0), 'name': 'Churned', 'itemStyle': {'color': '#EE6666'}}]}]}

            st_echarts(options=opt_donut, height='320px')

        # 2. customer behaviour pattern (retained vs churned)
        with row1_col2:
            ...
            display_names = ['App', 'Devices', 'Coupons', 'Satisfaction', 'Hike']
            ...
            for col in radar_cols:
                ...
                norm_0 = round(((val_0 - min_v) / (max_v - min_v) * 100) if max_v > min_v else 0, 1)
                norm_1 = round(((val_1 - min_v) / (max_v - min_v) * 100) if max_v > min_v else 0, 1)
            ...
            opt_bar = {
                ...
                'yAxis': {'type': 'value', 'name': 'Relative Score (0-100)', 'max': 100},
            ...
            st_echarts(options=opt_bar, height='320px')
        ...
        #1. churn rate by Ppoduct category
        with row2_col1:
            if 'PreferedOrderCat' in df.columns:
                st.subheader('Churn Rate by Product Category')
                cat_churn = (df.groupby('PreferedOrderCat')['Churn'].mean() * 100).round(1).sort_values(ascending=True)

                opt_cat = {'tooltip': {'trigger': 'axis', 'formatter': '{b}: {c}%'},
                           'xAxis': {'type': 'value', 'name': 'Churn Rate (%)'},
                           'yAxis': {'type': 'category', 'data': cat_churn.index.tolist()},
                           'series': [{'data': cat_churn.values.tolist(),
                                       'type': 'bar',
        ...
        with row2_col2:
            ...
            pay_churn = ((df.groupby('PreferredPaymentMode')['Churn'].mean() * 100).round(1).sort_values())
            opt_pay = {'tooltip': {'trigger': 'axis', 'formatter': '{b}: {c}%'},
        ...
        with row2_col3:
            ...
            tenure_churn = (df.groupby('TenureBucket')['Churn'].mean() * 100).round(1)
            opt_tenure = {'tooltip': {'trigger': 'axis', 'formatter': '{b}: {c}%'},
                          'xAxis': {'type': 'category', 'data': tenure_churn.index.tolist()},
                          'yAxis': {'type': 'value', 'name': 'Churn Rate (%)', 'max': 100},
                          'series': [{'data': tenure_churn.values.tolist(),
                                      'type': 'bar',
                                      'showBackground': True,
                                      'backgroundStyle': {'color': 'rgba(180, 180, 180, 0.1)'},
                                      'itemStyle': {'color': {'type': 'linear','x': 0, 'y': 0, 'x2': 0, 'y2': 1,
                          'colorStops': [{'offset': 0, 'color': '#EE6666'},
    ...
    with tab2:
        st.info('**What is ESG (Environmental, Social, and Governance) in this context?**\n\n'
                'In e-commerce operations, customer dissatisfaction, extended delivery distances, '
                'and product returns directly generate reverse logistics mileage, increasing overall carbon (CO2) emissions.')
        ...
        with col_chart2:
            ...
            opt_dist = {'tooltip': {'trigger': 'axis'},
                        'legend': {'data': ['No Complaint', 'Complaint']},
                        'xAxis': {'type': 'category', 'data': dist_comp.index.tolist()},
                        'yAxis': {'type': 'value', 'name': 'Customer Count'},
                        'series': [{'name': 'No Complaint', 'type': 'bar', 'stack': 'total', 'data': dist_comp[0].tolist(), 'itemStyle': {'color': '#91CC75'}},
        ...
        with sim_col1:
            complaint_reduction = st.slider('Target Complaint Reduction (%):',min_value=5, max_value=50, value=20, step=5)
        ...
        with sim_col2:
            ...
            sc1.metric('Estimated CO2 Saved', f'{saved_co2:.2f} Tons', delta=f'-{complaint_reduction}% Emissions')
            sc2.metric('Estimated Cost Savings', f'€{saved_cost:,.2f}', delta=f'-{complaint_reduction}% Expenses')
