...
try:
  ...
  with tab1:
    ...
    with row1_col2:
      ...
      if radar_cols and 'Churn' in df.columns:
        ...
        for col in radar_cols:
          ...
          norm_0 = round(((val_0 - min_v) / (max_v - min_v) * 100
                  if max_v > min_v
                  else 0),1,)
          
          norm_1 = round(((val_1 - min_v) / (max_v - min_v) * 100
                  if max_v > min_v
                  else 0),1,)
        ...
        opt_bar = {'tooltip': {'trigger': 'axis', 'axisPointer': {'type': 'shadow'}},
    ...
    with row2_col2:
      if 'PreferredPaymentMode' in df.columns and 'Churn' in df.columns:
        ...
        opt_pay = {'tooltip': {'trigger': 'axis', 'formatter': '{b}: {c}%'},
                   ...
                   'series': [{'data': pay_churn.values.tolist(),
                               'type': 'bar',
    ...
    with row2_col4:
      if 'SatisfactionScore' in df.columns and 'Churn' in df.columns:
        ...
        opt_sat = {'tooltip': {'trigger': 'axis', 'formatter': 'Score {b}: {c}%'},
                   'xAxis': {'type': 'category',
                   ...
                   'series': [{'data': sat_churn.values.tolist(),
                               'type': 'bar',
                               'itemStyle': {
                               'color': '#FAC858',
                               'borderRadius': [4, 4, 0, 0]}}]}
  ...
  with tab2:
    ...
    with col_chart2:
      ...
      if 'DistanceCategory' in df.columns and 'Complain' in df.columns:
        ...
        opt_dist = {'tooltip': {'trigger': 'axis'},
                    ...
                    'series': [{'name': 'No Complaint',
                                'type': 'bar',
                                'stack': 'total',
                                'data': (dist_comp[0].tolist() if 0 in dist_comp.columns else []),
                                'itemStyle': {'color': '#91CC75'}},
                               {'name': 'Complaint',
                                'type': 'bar',
                                'stack': 'total',
                                'data': (dist_comp[1].tolist() if 1 in dist_comp.columns else []),
                                'itemStyle': {'color': '#EE6666'}}]}
    ...
    with sim_col1:
      complaint_reduction = st.slider('Target Complaint Reduction (%):',
          min_value=5,
          max_value=50,
          value=20,
          step=5)
      
      cost_per_return = st.number_input('Average Logistics Cost per Return (€):', value=12.50)
  ...
  with tab3:
    ...
    with sim_right:
      ...
      if risk_score > 60:
        st.error('⚠️ High Risk Profile: Proactive customer retention intervention recommended.')
      else:
    ...
    st.markdown('See below how each factors pushes the risk score up (red) or down (blue).')
    ...
    try:
      ...
      if (hasattr(xgb_pipeline, 'named_steps')
        ...
        try:
          ...
          shap_values.feature_names = [name.split('__')[-1] for name in feature_names]
